...
async def initialize_providers():
    ...
    if missing_providers:
        logger.warning(
            f"Missing providers: {', '.join(missing_providers)}. Some demos might fail.",
            emoji_key="warning",
        )
        console.print(
            f"[yellow]Warning:[/yellow] Missing providers: {', '.join(missing_providers)}"
        )
    else:
...
async def execute_workflow(
    ...
    for i, step in enumerate(workflow_steps):
        ...
        if not tool_name:
            raise ValueError(
                f"Unsupported operation '{operation}' in workflow step '{step_id}'. Mapped tool name not found."
            )

        logger.info(
            f"Executing workflow step {i + 1}/{len(workflow_steps)}: ID='{step_id}', Tool='{tool_name}'"
        )
        ...
        if input_from_step:
            if input_from_step not in step_results:
                raise ValueError(
                    f"Input for step '{step_id}' requires output from '{input_from_step}', which has not run or failed."
                )
        ...
        elif tool_name in ["generate_completion", "chat_completion"] and isinstance(
            step_input_data, str
        ):
            if "prompt" not in parameters:
                parameters["prompt"] = (
                    step_input_data  # Assume input is the prompt if not specified
                )
        # Add more mappings as needed for other tools...
            ...
            logger.warning(
                f"Input mapping for tool '{tool_name}' not explicitly defined. Passing raw input."
            )
        ...
        try:
            ...
            if isinstance(step_output, dict):
                current_input_value = (
                    step_output.get("text")
                    or step_output.get("summary")
                    or step_output.get("result")
                    or step_output
                )
            else:
            ...
            logger.error(
                f"Error executing step '{step_id}' (Tool: {tool_name}): {e}", exc_info=True
            )
...
def display_task_analysis(title: str, result: Any):
    ...
    if "recommendations" in result and result["recommendations"]:
        rec_table = Table(
            title="[bold]Model Recommendations[/bold]", box=box.ROUNDED, show_header=True
        )
...
async def run_analyze_task_demo():
    ...
    task_description = (
        "Summarize the provided technical document about AI advancements and extract key entities."
    )
...
async def run_delegate_task_demo(tracker: CostTracker):  # Add tracker
    ...
    logger.info(
        "Running task delegation demo (using recommend_model + completion)...", emoji_key="start"
    )
    ...
    for priority in priorities:
        ...
        try:
            ...
            recommendation_result_raw = await mcp.call_tool(
                "recommend_model",
                {
            ...
            completion_result_raw = await mcp.call_tool(
                "generate_completion",
                {"prompt": prompt, "provider": rec_provider, "model": rec_model, "max_tokens": 100},
            )
            ...
            if (
                ...
                and all(k in completion_result for k in ["cost", "provider", "model"])
                and "tokens" in completion_result
            ):
                try:
                    trackable = TrackableResult(
                        ...
                        provider=completion_result.get(
                            "provider", rec_provider
                        ),  # Use known provider as fallback
                        model=completion_result.get(
                            "model", rec_model
                        ),  # Use known model as fallback
                        processing_time=completion_result.get("processing_time", 0.0),
                    )
                    tracker.add_call(trackable)
                except Exception as track_err:
                    logger.warning(
                        f"Could not track cost for delegated task ({priority}): {track_err}",
                        exc_info=False,
                    )
            ...
            if "error" in completion_result:
                logger.error(
                    f"Completion failed for recommended model {rec_model}: {completion_result['error']}"
                )
                console.print(
                    f"[red]Completion failed for {rec_model}: {completion_result['error']}[/red]"
                )
            else:
                console.print(
                    Panel(
            ...
            logger.error(
                f"Error delegating task with priority {priority}: {e}",
                emoji_key="error",
                exc_info=True,
            )
...
async def run_workflow_demo():
    ...
    try:
        workflow_json = json.dumps(
            workflow, indent=2, default=lambda o: o.value if isinstance(o, Provider) else str(o)
    ...
    try:
        result = await mcp.call_tool(
            "execute_workflow", {"workflow_steps": workflow, "initial_input": initial_text}
        )
        ...
        console.print(
            Panel(
                escape(
                    ...
                    if optimized_prompt
                    else "[red]Optimization failed[/red]"
                ),
