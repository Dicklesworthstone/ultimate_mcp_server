...
async def _execute_with_recommended_model(
    balanced_rec, prompt, estimated_output_tokens, tracker: CostTracker
):
    ...
    if not provider_name:
        logger.error(
            f"Could not determine provider for recommended model {balanced_rec}", emoji_key="error"
        )
        return
    ...
    try:
        ...
        result = await provider.generate_completion(
            prompt=prompt, model=balanced_rec, temperature=0.7, max_tokens=estimated_output_tokens
        )
        ...
        logger.info(
            f"Actual input tokens: {result.input_tokens}, output tokens: {result.output_tokens}",
            emoji_key="token",
        )
        ...
        logger.error(
            f"Error running completion with {balanced_rec}: {e}", emoji_key="error", exc_info=True
        )
...
async def demonstrate_cost_optimization(tracker: CostTracker):
    ...
    for model_name_only in models_to_compare:  # Renamed variable for clarity
        try:
            ...
            if not provider_name:
                logger.warning(
                    f"Could not determine provider for model '{model_name_only}'. Skipping cost estimation.",
                    emoji_key="warning",
                )
                cost_table.add_row(
                    escape(model_name_only),
                    "-",
                    "-",
                    "-",
                    "-",
                    "[dim yellow]Unknown provider[/dim yellow]",
                )
                continue
            ...
            if "error" in estimate_result:
                ...
                logger.warning(
                    f"Could not estimate cost for {model_name_only}: {estimate_result['error']}",
                    emoji_key="warning",
                )
                cost_table.add_row(
                    escape(model_name_only),
                    ...
                    "-",
                    "-",
                    f"[dim red]{estimate_result['error']}[/dim red]",
                )
            else:
            ...
            logger.error(
                f"Error calling estimate_cost for {model_name_only}: {e}",
                emoji_key="error",
                exc_info=True,
            )
    ...
    logger.info(
        "Getting model recommendations based on different priorities", emoji_key="recommend"
    )
    ...
    task_info = {
        ...
        "expected_output_length": estimated_output_tokens
        * 4,  # Convert tokens back to chars approx
    ...
    for priority in priorities:
        try:
            ...
            raw_result = await mcp.call_tool("recommend_model", {**task_info, "priority": priority})
            ...
            if "error" in rec_result:
                logger.warning(
                    f"Could not get recommendations for priority '{priority}': {rec_result['error']}",
                    emoji_key="warning",
                )
                recommendation_table.add_row(
                    priority, "-", "-", "-", "-", "-", f"[dim red]{rec_result['error']}[/dim red]"
                )
            elif not rec_result.get("recommendations"):
                logger.info(f"No models met criteria for priority '{priority}'", emoji_key="info")
                recommendation_table.add_row(
                    priority, "[dim]None[/dim]", "-", "-", "-", "-", "No models fit criteria"
                )
            else:
                recs = rec_result["recommendations"]
                top_rec = recs[0]
                other_recs_str = (
                    ", ".join([escape(r["model"]) for r in recs[1:]]) if len(recs) > 1 else "None"
                )
                ...
                if "score" not in top_rec:
                    if priority == "cost":
                        ...
                        score = 10.0 / (
                            float(top_rec.get(cost_key, 1.0)) + 0.001
            ...
            logger.error(
                f"Error calling recommend_model for priority {priority}: {e}",
                emoji_key="error",
                exc_info=True,
            )
    ...
    try:
        ...
        if "balanced" in recommendation_results:
            ...
            raw_result = await mcp.call_tool(
                "recommend_model", {**task_info, "priority": "balanced"}
            )
    ...
    if balanced_rec:
        console.print(
            Rule(f"[cyan]Executing with Recommended Model ({escape(balanced_rec)})[/cyan]")
        )
        logger.info(
            f"Running completion with balanced recommendation: {balanced_rec}",
            emoji_key="processing",
        )

        # Use the new helper function instead of direct API key handling
        await _execute_with_recommended_model(
            balanced_rec, prompt, estimated_output_tokens, tracker
        )
    else:
