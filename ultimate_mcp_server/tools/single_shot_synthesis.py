...
STORAGE_DIR_BASE = (
    Path(__file__).resolve().parent.parent.parent / "storage" / "single_shot_synthesis"
)
...
async def _generate_expert_response(
    ...
    if derived_provider == "anthropic" and current_max_tokens is None:
        logger.info(
            f"Anthropic model {config.model_id} called without max_tokens, defaulting to 2048 for timeout calculation."
        )
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            response_data.metrics["output_tokens"] = completion_result.get("tokens", {}).get(
                "output"
            )
            response_data.metrics["api_latency_ms"] = int(
                completion_result.get("processing_time", 0) * 1000
            )
            response_data.metrics["api_model_id_used"] = completion_result.get(
                "model", config.model_id
            )
            ...
            if completion_result.get("success"):
                ...
                response_data.error = completion_result.get(
                    "error", f"Generation failed on attempt {attempt + 1}"
                )
                if attempt == max_retries:  # Last attempt
                    logger.error(
                        f"Expert {config.model_id} failed after {max_retries + 1} attempts: {response_data.error}"
                    )

        except Exception as e:
            logger.error(
                f"Exception during expert call {config.model_id} (attempt {attempt + 1}): {e}",
                exc_info=True,
            )
            response_data.error = str(e)
            if attempt == max_retries:
                logger.error(
                    f"Expert {config.model_id} failed with exception after {max_retries + 1} attempts."
                )
        ...
        if attempt < max_retries:
            ...
            logger.info(
                f"Expert {config.model_id} attempt {attempt + 1} failed. Retrying in {sleep_duration:.2f}s..."
            )
...
async def single_shot_synthesis(
    ...
    try:
        ...
        output_data.total_metrics["total_task_time_ms"] = int(
            (time.monotonic() - task_start_time) * 1000
        )
        logger.error(f"SingleShotSynthesis input validation error for '{name}': {e.json(indent=2)}")
        raise ToolError(
            f"Invalid input for single_shot_synthesis: {e.errors()}", status_code=400
        ) from e

    # 1. Parallel fan-out to expert models
    logger.info(
        f"[{request_id}] Starting expert model responses for '{name}'. Count: {len(parsed_expert_configs)}"
    )
    expert_tasks = [
        _generate_expert_response(prompt, config, retry_cfg_experts)
        for config in parsed_expert_configs
    ]
    output_data.expert_responses = await asyncio.gather(
        *expert_tasks, return_exceptions=False
    )  # Exceptions handled in _generate_expert_response
    ...
    agg_prompt_parts = [
        f"You are an advanced AI tasked with synthesizing a definitive answer based on multiple expert inputs.\n\nOriginal Problem/Prompt:\n---\n{prompt}\n---\n"
    ]
    agg_prompt_parts.append(
        "Below are responses from several expert models. Review them critically:\n"
    )
    ...
    logger.info(
        f"[{request_id}] Requesting synthesis from {parsed_synthesizer_config.model_id} for '{name}'."
    )
    ...
    try:
        ...
        if synth_response_raw.response_text and not synth_response_raw.error:
            ...
            output_data.synthesizer_thinking_process = await extract_thinking(
            ...
            if output_data.synthesizer_thinking_process and output_data.synthesized_response_text:
                ...
                output_data.synthesized_response_text = (
                    output_data.synthesized_response_text.replace(
                        f"<thinking>{output_data.synthesizer_thinking_process}</thinking>", ""
                    ).strip()
                )
            ...
            if tournament_type == "code":
                ...
                if code_extraction_result.get("success"):
                    output_data.synthesized_extracted_code = code_extraction_result.get(
                        "code_block"
                    )
                else:  # Log if extraction failed but don't mark whole thing as failure
                    logger.warning(
                        f"[{request_id}] Code extraction from synthesizer output failed: {code_extraction_result.get('error')}"
                    )
    ...
    if tournament_type == "code" and output_data.synthesized_extracted_code:
        ...
        output_data.synthesized_response_text
    ):  # Fallback to full text if code not extracted or text type
    ...
    if tournament_type == "code" and output_data.synthesized_extracted_code:
        (storage_path / "synthesized_code.py").write_text(
            output_data.synthesized_extracted_code, encoding="utf-8"
        )
    ...
    output_data.status = (
        ...
        if synthesizer_success
            ...
            "PARTIAL_SUCCESS"
            if any(er.response_text for er in output_data.expert_responses)
            else "FAILED"
        )
    )
    if (
        not synthesizer_success and not output_data.error_message
    ):  # General failure if no specific error
        output_data.error_message = (
            output_data.error_message or "Synthesis process encountered an issue."
        )
    ...
    (storage_path / "overall_metrics.json").write_text(
        json.dumps(output_data.total_metrics, indent=2), encoding="utf-8"
    )

    logger.info(
        f"[{request_id}] Single-shot synthesis '{name}' completed. Status: {output_data.status}. Cost: ${total_cost_agg:.4f}"
    )
