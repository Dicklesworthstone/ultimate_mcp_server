            ...
            if allow_creation and isinstance(result, dict) and "resource_id" in result:
                ...
                if resource_registry is not None:
                    ...
                    await resource_registry.register(
                        resource_type, new_resource_id, metadata=metadata
                    )
        ...
        if self.storage:
            try:
                await self.storage.save_resource(
                    resource_type, resource_id, self.resources[resource_type][resource_id]
                )
        ...
        if self.storage:
            try:
                resources = await self.storage.list_resources(
                    resource_type, limit=limit, offset=offset, filters=filters
                )
        ...
        if resource_type in self.resources:
            ...
            if filters:
                for key, value in filters.items():
                    filtered_resources = [
                        r for r in filtered_resources if r.get("metadata", {}).get(key) == value
                    ]
...
class BaseTool:
    ...
    async def execute(self, tool_name: str, params: Dict[str, Any]) -> Any:
        ...
        if not method or not hasattr(method, "_tool"):
            raise ToolError(f"Tool not found: {tool_name}", error_code="tool_not_found")

        # Execute tool with metrics wrapper
        return await self._wrap_with_metrics(method, **params)

    async def _wrap_with_metrics(self, func: Callable, *args, **kwargs) -> Any:
        """
        ...
        try:
            ...
            self.metrics.record_call(success=success, duration=duration, tokens=tokens, cost=cost)
        ...
        try:
            ...
            if self_obj and hasattr(self_obj, "metrics"):
                self_obj.metrics.record_call(
                    success=success, duration=duration, tokens=tokens, cost=cost
                )
...
def with_error_handling(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.error(specific_message, emoji_key="error", tool=tool_name, exc_info=True)
