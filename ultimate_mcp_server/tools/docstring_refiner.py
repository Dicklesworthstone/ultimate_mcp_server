...
TestStrategy: TypeAlias = Literal[
    "positive_required_only",
    "positive_optional_mix",
    "positive_all_optional",
    "negative_type",
    "negative_required",
    "negative_enum",
    "negative_format",
...
class TestCaseGenerationStrategy(BaseModel):
    ...
    positive_required_only: int = Field(
        default=2, ge=0, description="Number of tests using only required parameters."
    )
    ...
    negative_required: int = Field(
        default=2, ge=0, description="Number of tests deliberately missing one required parameter."
    )
...
def _apply_schema_patches(
    base_schema: JsonDict, patch_suggestions: List[ParameterSchemaPatch]
            ...
            if "properties" in result_schema and "required" in result_schema:
                ...
                if missing_req:
                    raise JsonPatchException(
                        f"Patched schema invalid: Required properties {missing_req} not defined."
                    )
...
def _create_value_for_schema(
    schema: JsonDict, strategy: str = "positive", history: Optional[List[Any]] = None
) -> Any:
    """
    ...
    if (
        strategy.startswith("negative_constraint")
        or strategy.startswith("negative_enum")
        or strategy.startswith("negative_format")
        or strategy.startswith("negative_range")
        or strategy.startswith("negative_length")
        or strategy.startswith("negative_pattern")
    ):
    ...
    if schema_type == "string":
        ...
        val = [
            _create_value_for_schema(item_schema, "positive", history + [num]) for _ in range(num)
        ]
...
def _select_test_strategies(
    ...
    if not props:  # Very simple schema (e.g., just a single root type)
        ...
        for k in list(weights.keys()):  # Reduce weight of complex strategies
            if (
                k.startswith("negative_constraint")
                or k.startswith("llm_")
                or k == "positive_optional_mix"
                or k == "positive_all_optional"
            ):
                weights[k] *= 0.1
    else:
        ...
        if total_failures > 0:
            ...
            for strategy, fail_count in strategy_failures.items():
                if strategy in weights:
                    weights[strategy] *= (
                        1 + (fail_count / total_failures) * 2.0
                    )  # Strong boost for failing strategies
            ...
            for code, count in error_codes.items():
                ...
                if (
                    "VALIDATION_ERROR" in code
                    or "INVALID_PARAMETER" in code
                    or code == "SCHEMA_VALIDATION_FAILED_PRE_CALL"
                ):
                    ...
                    "PROVIDER_ERROR" in code
                    or "TOOL_EXECUTION_FAILED" in code
                    or "UNEXPECTED" in code
                ):
    ...
    if total_weight <= 0:  # Prevent division by zero
        ...
        logger.debug(
            f"Selected test strategies (Even distribution due to zero total weight): {final_counts}"
        )
        return final_counts
    ...
    if final_sum != num_tests:
        logger.warning(
            f"Final test count ({final_sum}) doesn't match requested ({num_tests}). Adjusting..."
        )
...
async def _generate_test_cases(
    ...
    logger.debug(
        f"Selecting test strategies for {tool_name} based on schema and previous results..."
    )
    ...
    try:
        ...
        logger.error(
            f"Failed to select test strategies for {tool_name}: {e}. Falling back to basic.",
            exc_info=True,
        )
        # Fallback strategy: focus on positive cases
        strategy_counts = [
            ("positive_required_only", max(1, num_tests // 2)),
    ...
    def add_unique_test_case(
        strategy: TestStrategy, args: JsonDict, description: str, targets_failure: bool = False
    ):
        ...
        if args_hash not in seen_args_hashes:
            ...
            if should_validate and schema_properties:
                ...
                if validation_error:
                    logger.debug(
                        f"Schema validation failed for generated args (Strategy: {strategy}): {validation_error}. Args: {args}"
                    )
            ...
            while generated_count < count and attempts < max_attempts:
                ...
                try:
                    if strategy == "positive_required_only":
                        case = {
                            p: _create_value_for_schema(param_schemas[p], "positive")
                        ...
                        if strategy_type == "type":
                            ...
                            suitable_params = [
                                p
                                for p, s in param_schemas.items()
                            ...
                            suitable_params = [
                                p for p, s in param_schemas.items() if "pattern" in s
                            ]
                        elif strategy_type == "empty":
                            suitable_params = [
                                p
                                for p, s in param_schemas.items()
                                if s.get("type") in ["string", "array", "object"]
                            ]
                            ...
                            suitable_params = [
                                ...
                                for p, s in param_schemas.items()
                                if s.get("type") == "null"
                                or "null" in s.get("type", [])
                                or p not in required_params
                            ]
                        elif strategy_type == "boundary_min":
                            suitable_params = [
                                p for p, s in param_schemas.items() if "minimum" in s
                            ]
                        elif strategy_type == "boundary_max":
                            suitable_params = [
                                p for p, s in param_schemas.items() if "maximum" in s
                            ]

                        if not suitable_params:
                            continue  # Cannot apply this strategy
                        targeted_param = random.choice(suitable_params)
                        value = _create_value_for_schema(
                            param_schemas[targeted_param], strategy
                        )  # Pass full strategy name
                        ...
                        for pname in required_params:
                            if pname != targeted_param:
                                case[pname] = _create_value_for_schema(
                                    param_schemas[pname], "positive"
                                )
                        strategy_desc = (
                            f"Strategy '{strategy}' targeting parameter '{targeted_param}'."
                        )
                        targets_failure = strategy.startswith(
                            "negative_"
                        )  # Negative strategies target failure

                    # Add the generated case if valid for the strategy
                    if case is not None:  # Ensure case was generated
                        if add_unique_test_case(
                            cast("TestStrategy", strategy), case, strategy_desc, targets_failure
                        ):
                            generated_count += 1
                except Exception as gen_err:
                    logger.warning(
                        f"Error generating test case for strategy '{strategy}' on tool '{tool_name}': {gen_err}",
                        exc_info=True,
                    )
        ...
        try:
            result = await generate_completion(
                ...
                **refinement_model_config,
                ...
                if refinement_model_config.get("provider") == Provider.OPENAI.value
                else None,
            )
            if not result.get("success"):
                logger.warning(
                    f"LLM failed test gen ({llm_strategy}) for {tool_name}: {result.get('error')}"
                )
                ...
                if isinstance(llm_args_list, list):
                    for item in llm_args_list:
                        if isinstance(item, dict):
                            ...
                            add_unique_test_case(
                                cast(TestStrategy, llm_strategy),
                                item,
                                f"LLM generated ({llm_strategy})",
                                targets_failure=False,
                            )
            except (json.JSONDecodeError, TypeError) as e:
                logger.error(
                    f"Failed parse LLM ({llm_strategy}) args: {e}. Raw: {args_text}", exc_info=False
                )
        except Exception as e:
            logger.error(
                f"Error during LLM ({llm_strategy}) test gen for {tool_name}: {e}", exc_info=True
            )

    # --- 4. Ensure Minimum Tests & Apply Cap ---
    if not test_cases:
        logger.warning(
            f"No test cases generated for {tool_name}, creating a default positive case."
        )
        # Create a simple default positive case if nothing else worked
        default_case = (
            {p: _create_value_for_schema(param_schemas[p], "positive") for p in required_params}
            if required_params
            else {}
        )
        add_unique_test_case(
            "positive_required_only", default_case, "Default positive case (fallback)"
        )

    # Apply the final cap based on the original num_tests request
    final_cases = test_cases[:num_tests]
    logger.debug(
        f"Final generated {len(final_cases)} test cases for tool '{tool_name}' (requested: {num_tests})."
    )
...
async def _execute_tool_tests(
    mcp_instance: Any, tool_name: str, test_cases: List[TestCase]
    ...
    async def run_single_test(test_case: TestCase) -> Tuple[TestExecutionResult, float]:
        ...
        async with semaphore:
            ...
            if skip_execution:
                ...
                logger.debug(
                    f"Skipping execution for test case (Strategy: {test_case.strategy_used}) due to pre-validation error: {test_case.arguments}"
                )
            else:
                # --- Execute the tool ---
                try:
                    call_result = await mcp_instance.call_tool(
                        tool_name=tool_name, arguments=test_case.arguments
                    )
                    ...
                    if isinstance(call_result, dict):
                        ...
                        if is_mcp_tool_error or is_jsonrpc_error:
                            ...
                            if not isinstance(error_data, dict):  # Handle string errors
                                ...
                                error_message = error_data.get(
                                    "message", "Tool reported an unknown error."
                                )
                        ...
                        success = True
                        try:
                            result_preview = repr(call_result)[:80] + (
                                "..." if len(repr(call_result)) > 80 else ""
                            )
                        except Exception:
                ...
                # --- Handle Exceptions During Execution ---
                    ...
                    logger.warning(
                        f"Tool execution caught known error for {tool_name}: {error_message}"
                    )
                except Exception as exec_err:
                    error_message = (
                        f"Unexpected Execution Error: {type(exec_err).__name__}: {str(exec_err)}"
                    )
                    error_type = type(exec_err).__name__
                    error_code = "UNEXPECTED_EXECUTION_ERROR"
                    error_details = {"traceback": traceback.format_exc()}
                    logger.error(
                        f"Tool execution caught unexpected error for {tool_name}: {error_message}",
                        exc_info=True,
                    )
    ...
    for i, res_or_exc in enumerate(results_with_costs):
        ...
        if isinstance(res_or_exc, Exception):
            ...
            logger.error(
                f"Gather caught exception for test case {i} (Strategy: {test_case_used.strategy_used}): {res_or_exc}",
                exc_info=res_or_exc,
            )
            # Guess the error category based on the exception message
            error_category = (
                ErrorCategory.TOOL_EXECUTION_ERROR
                if "tool" in str(res_or_exc).lower()
            ...
            fail_result = TestExecutionResult(
                ...
                error_details={
                    "traceback": traceback.format_exception(
                        type(res_or_exc), res_or_exc, res_or_exc.__traceback__
                    )
                },
            ...
            logger.error(
                f"Unexpected result type from gather for test case {i}: {type(res_or_exc)}"
            )
            fail_result = TestExecutionResult(
                test_case=test_case_used,
                start_time=time.time(),
    ...
    logger.debug(
        f"Executed {len(final_results)} tests for tool '{tool_name}'. Accumulated cost: ${accumulated_cost:.6f}"
    )
...
async def _generate_examples(
    ...
    if not schema.get("properties") and not success_cases and not failure_cases:
        logger.debug(
            f"Skipping example generation for '{tool_name}': No properties, successes, or failures provided."
        )
        return [], cost
    if not description and not schema.get("properties"):
        logger.debug(
            f"Skipping example generation for '{tool_name}': No description or properties."
        )
        return [], cost
    ...
    success_examples_str = "\n".join(
        [
            f"- Args: {json.dumps(r.test_case.arguments, default=str)}"
    ...
    if failure_cases:
        failure_examples_str += (
            "Examples of Failed Calls (Args -> Error Type: Error Code -> Message):\n"
        )
...
"""
    ...
    try:
        logger.debug(
            f"Generating examples for tool '{tool_name}' using model '{refinement_model_config.get('model')}'..."
        )
        result = await generate_completion(
            prompt=prompt,
            **refinement_model_config,
        ...
        try:
                    ...
                    if isinstance(ex_data, dict):
                        try:
                            ...
                            if validation_level != "none":
                                validation_error = _validate_args_against_schema(
                                    validated_ex.args, schema
                                )
                                if validation_error:
                                    logger.warning(
                                        f"LLM generated example {i + 1} for '{tool_name}' failed schema validation: {validation_error}. Skipping example."
                                    )
                                    continue  # Skip examples that don't validate against the schema
                            validated_examples.append(validated_ex)
                        except ValidationError as pydantic_err:
                            logger.warning(
                                f"Skipping invalid example structure at index {i} for '{tool_name}': {pydantic_err}. Data: {ex_data}"
                            )
                        except Exception as val_err:  # Catch other potential validation errors
                            logger.warning(
                                f"Error processing example item {i} for '{tool_name}': {val_err}. Data: {ex_data}"
                            )
                    else:
                        logger.warning(
                            f"Skipping non-dictionary item in examples list for '{tool_name}' at index {i}."
                        )
            else:
                # LLM did not return a list as requested
                raise ValueError(
                    f"LLM response was not a JSON list as expected. Type: {type(examples_list_raw)}"
                )

            logger.info(
                f"Successfully generated and validated {len(validated_examples)} examples for '{tool_name}'."
            )
            ...
            logger.error(
                f"Failed to parse or validate LLM examples JSON for '{tool_name}': {e}. Raw response: {examples_text}",
                exc_info=True,
            )
...
async def _analyze_and_propose(
    ...
    logger.debug(
        f"Iter {iteration}, Tool '{tool_name}': Generating examples based on {len(success_cases)} successes and {len(failures)} failures."
    )
    try:
        ...
        logger.debug(
            f"Iter {iteration}, Tool '{tool_name}': Generated {len(generated_examples)} examples (Cost: ${example_cost:.6f})."
        )
    except Exception as e:
        logger.error(
            f"Iter {iteration}, Tool '{tool_name}': Failed to generate examples: {e}", exc_info=True
        )
    ...
    if not failures:
        logger.info(
            f"Iter {iteration}, Tool '{tool_name}': No failures detected. Skipping analysis, proposing current docs with new examples."
        )
    ...
    error_summary_text = ""
    error_patterns: Dict[ErrorPatternKey, Dict[str, Any]] = defaultdict(
    ...
    error_summary_text += (
        "Observed Failure Patterns (Grouped by ErrorType:ErrorCode:ParamTarget):\n"
    )
    for pattern, info in error_patterns.items():
        error_summary_text += f"- Pattern '{pattern}' ({info['count']} occurrences):\n"
        for ex in info["examples"]:
            val_err = (
                f" (SchemaPreFail: {ex['validation_error']})" if ex["validation_error"] else ""
            )
            details_preview = (
                f" Details: {json.dumps(ex['details'], default=str)[:60]}..."
                if ex["details"]
                else ""
            )
...
JSON Output:"""
    ...
    async def run_single_analysis(
        ...
        try:
            logger.debug(
                f"Sending analysis prompt (Iter {iteration}, Tool '{tool_name}') to {model_identifier}"
            )
            result = await generate_completion(
                prompt=prompt,
                **config,
                temperature=0.1,
                max_tokens=4000,  # Generous token limit
                additional_params={"response_format": {"type": "json_object"}}
                if config.get("provider") == Provider.OPENAI.value
                else None,
            )
            cost = result.get("cost", 0.0)
            if not result.get("success"):
            ...
            try:
                ...
                logger.error(
                    f"Failed parse/validate analysis JSON from {model_identifier}: {e}. Raw: {analysis_text}",
                    exc_info=True,
                )
                diag = f"Parse Error from {model_identifier}: {e}. Raw: {analysis_text[:200]}..."
                analysis_model = RefinementAnalysis(
                    overall_diagnosis=diag,
                    improvement_confidence=0.0,
                    hypothetical_error_resolution="Parse failed",
                )
                prop_changes = ProposedChanges(
                    description=current_docs.description, examples=generated_examples
                )  # Fallback
        except Exception as e:
            logger.error(
                f"Error during analysis execution with {model_identifier}: {e}", exc_info=True
            )
            diag = f"Failed execution for {model_identifier}: {e}"
            analysis_model = RefinementAnalysis(
                overall_diagnosis=diag,
                improvement_confidence=0.0,
                hypothetical_error_resolution="Execution failed",
            )
            prop_changes = ProposedChanges(
                description=current_docs.description, examples=generated_examples
            )
    ...
    best_proposed_changes: Optional[ProposedChanges] = ProposedChanges(
        ...
        schema_patches=[],  # Start with current + new examples
    )
    ...
    if best_proposed_changes is None:
        best_proposed_changes = ProposedChanges(
            description=current_docs.description, examples=generated_examples, schema_patches=[]
        )
...
async def _simulate_agent_usage(
    ...
    examples_str = (
        json.dumps([ex.model_dump() for ex in current_docs.examples[:1]], indent=2)
        if current_docs.examples
...
"""
        try:
            ...
            try:
                ...
                sim_result_obj = AgentSimulationResult(
                    ...
                    formulation_success=sim_data.get(
                ...
                logger.error(
                    f"Failed to parse/validate simulation result JSON for task '{task_desc}': {e}. Raw: {sim_text}",
                    exc_info=True,
                )
            ...
            # Catch errors during the generate_completion call or other unexpected issues
            logger.error(
                f"Error during agent simulation LLM call for task '{task_desc}': {e}", exc_info=True
            )
            sim_results.append(
                AgentSimulationResult(
                    task_description=task_desc,
    ...
    logger.info(
        f"Agent simulation completed for tool '{tool_name}' ({len(sim_results)} scenarios)."
    )
...
"""
    ...
    try:
        logger.debug(
            f"Sending winnowing prompt for '{tool_name}' to {refinement_model_config.get('model')}"
        )
        result = await generate_completion(
            prompt=prompt,
            **refinement_model_config,
        ...
        try:
            ...
            winnowed_proposal = ProposedChanges(
                ...
                examples=[
                    GeneratedExample(**ex)
                    for ex in winnow_data.get("examples", [])
            ...
            if len(winnowed_proposal.examples) > 3:  # Allow maybe 3 max, even if asked for 1-2
                logger.warning(
                    f"Winnowing returned {len(winnowed_proposal.examples)} examples, expected 1-2. Keeping all."
                )
            ...
            logger.error(
                f"Failed to parse/validate winnowing JSON for '{tool_name}': {e}. Raw: {winnow_text}",
                exc_info=True,
            )
        ...
        logger.error(
            f"Error during winnowing LLM call or processing for '{tool_name}': {e}", exc_info=True
        )
...
async def refine_tool_documentation(
    ...
    validation_level: Optional[
        Literal["none", "basic", "full"]
    ] = None,  # Made optional, default below
    ...
    if not all_tools_dict:
        ...
        return DocstringRefinementResult(
            ...
            total_test_calls_attempted=0,
            total_test_calls_failed=0,
            total_schema_validation_failures=0,
            total_agent_simulation_failures=0,
    ...
    if not refine_all_available and (not tool_names or not isinstance(tool_names, list)):
        raise ToolInputError(
            "Either 'tool_names' list must be provided or 'refine_all_available' must be True."
        )
    ...
    if missing:
        raise ToolInputError(
            f"Tools not found: {', '.join(missing)}. Available: {list(all_tools_dict.keys())}"
        )
    ...
    num_tests_per_tool = min(
        sum(getattr(test_gen_strategy, f) for f in test_gen_strategy.model_fields),
        MAX_TEST_QUERIES_PER_TOOL,
    )
    ...
    if analysis_ensemble_configs:
        analysis_configs.extend(
            [
                c
                for c in analysis_ensemble_configs
                if isinstance(c, dict) and c != primary_refinement_config
            ]
        )
    ...
    async def process_single_tool(tool_name: str) -> RefinedToolResult:
        nonlocal \
            total_refinement_cost_overall, \
            total_tests_attempted_overall, \
            total_tests_failed_overall
        nonlocal \
            total_schema_validation_failures, \
            total_agent_simulation_failures, \
            total_iterations_run_all_tools
        ...
        if not tool_def or not hasattr(tool_def, "name"):
            return RefinedToolResult(
                tool_name=tool_name,
                iterations=[],
                original_schema={},
                final_proposed_changes=ProposedChanges(
                    description="ERROR: Tool definition missing"
                ),
                final_schema_after_patches={},
                initial_success_rate=0,
                final_success_rate=0,
                improvement_factor=0,
                process_error="Tool definition missing",
            )
        ...
        if not isinstance(original_schema, dict):
            return RefinedToolResult(
                ...
                final_success_rate=0,
                improvement_factor=0,
                process_error="Invalid original schema",
            )

        tool_iterations_results: List[RefinementIterationResult] = []
        current_proposed_changes = ProposedChanges(
            description=original_desc, examples=[]
        )  # Start with original desc
        ...
        try:
            for iteration in range(1, max_iterations + 1):
                ...
                schema_before_patch = copy.deepcopy(
                    current_schema_for_iter
                )  # Schema used for this iter
                ...
                await _emit_progress(
                    {
                        "tool_name": tool_name,
                        "iteration": iteration,
                        "stage": "agent_simulation",
                        "message": "Simulating agent usage...",
                    }
                )
                agent_sim_results, sim_cost = await _simulate_agent_usage(
                    tool_name,
                    current_proposed_changes,
                    current_schema_for_iter,
                    primary_refinement_config,
                )
                total_refinement_cost_overall += sim_cost
                current_agent_sim_failures = sum(
                    1 for r in agent_sim_results if not r.formulation_success
                )
                ...
                await _emit_progress(
                    {
                        ...
                        "iteration": iteration,
                        "stage": "agent_simulation",
                        "message": f"Simulation complete ({current_agent_sim_failures} failures).",
                    }
                )
                ...
                previous_results_for_guidance = (
                    tool_iterations_results[-1].test_results if tool_iterations_results else None
                )
                test_cases, test_gen_cost = await _generate_test_cases(
                    tool_name=tool_name,
                    tool_schema=current_schema_for_iter,
                    tool_description=current_proposed_changes.description,
                    num_tests=num_tests_per_tool,
                ...
                valid_test_cases_for_exec = [
                    tc
                    for tc in test_cases
                    if eff_validation_level != "full"
                    or tc.schema_validation_error is None
                    or tc.strategy_used.startswith("negative_")
                ]
                await _emit_progress(
                    {
                        "tool_name": tool_name,
                        "iteration": iteration,
                        "stage": "test_generation",
                        "message": f"Generated {len(test_cases)} cases ({iter_validation_failures} validation fails). Executing {len(valid_test_cases_for_exec)}.",
                    }
                )
                ...
                await _emit_progress(
                    {
                        "tool_name": tool_name,
                        "iteration": iteration,
                        "stage": "test_execution_start",
                        "message": f"Executing {len(valid_test_cases_for_exec)} tests...",
                    }
                )
                test_results, test_exec_cost = await _execute_tool_tests(
                    mcp_instance=mcp_instance,
                    tool_name=tool_name,
                    test_cases=valid_test_cases_for_exec,
                )
                ...
                current_success_rate = (
                    (len(test_results) - current_failures) / len(test_results)
                    if test_results
                    else 1.0
                )
                if iteration == 1:
                ...
                await _emit_progress(
                    {
                        "tool_name": tool_name,
                        "iteration": iteration,
                        "stage": "test_execution_end",
                        "message": f"Execution complete ({current_failures} failures).",
                        "details": {
                ...
                iter_proposal_changes: Optional[ProposedChanges] = (
                    current_proposed_changes.model_copy(deep=True)
                )
                ...
                if current_failures > 0 or iteration == 1:
                    await _emit_progress(
                        {
                            ...
                            "message": "Analyzing failures and proposing improvements...",
                        }
                    )
                    (
                        iter_analysis,
                        iter_proposal_changes,
                        analysis_cost,
                    ) = await _analyze_and_propose(
                        tool_name=tool_name,
                        iteration=iteration,
                        current_docs=current_proposed_changes,
                        current_schema=current_schema_for_iter,
                    ...
                    iter_schema_patches = (
                        iter_proposal_changes.schema_patches if iter_proposal_changes else []
                    )
                    await _emit_progress(
                        {
                            "tool_name": tool_name,
                            "iteration": iteration,
                            "stage": "analysis_end",
                            "message": f"Analysis complete (Confidence: {iter_analysis.improvement_confidence:.2f if iter_analysis else 'N/A'}).",
                        }
                    )
                else:
                    iter_analysis = RefinementAnalysis(
                        overall_diagnosis="No errors this iteration.",
                        improvement_confidence=1.0,
                        hypothetical_error_resolution="N/A",
                    )
                    ...
                    await _emit_progress(
                        {
                            "tool_name": tool_name,
                            "iteration": iteration,
                            "stage": "schema_patching",
                            "message": f"Applying {len(iter_schema_patches)} schema patches...",
                        }
                    )
                    schema_after_patch, schema_apply_errors, applied_patches_this_iter = (
                        _apply_schema_patches(schema_before_patch, iter_schema_patches)
                    )
                        ...
                        await _emit_progress(
                            {
                                ...
                                "iteration": iteration,
                                "stage": "error",
                                "message": f"Schema patch errors: {schema_apply_errors}",
                            }
                        )
                    accumulated_patches.extend(applied_patches_this_iter)

                # 6. Calculate Diffs
                iter_diff_desc = _create_diff(
                    current_proposed_changes.description,
                    iter_proposal_changes.description,
                    f"desc_iter_{iteration - 1}",
                    f"desc_iter_{iteration}",
                )
                ...
                iter_result = RefinementIterationResult(
                    ...
                    test_results=test_results,
                    success_rate=current_success_rate,
                    validation_failure_rate=(iter_validation_failures / len(test_cases))
                    if test_cases
                    else 0.0,
                    analysis=iter_analysis,
                    proposed_changes=iter_proposal_changes.model_copy(deep=True)
                    if iter_proposal_changes
                    else None,
                ...
                if stop_reason:
                    logger.info(
                        f"Iter {iteration}: Stopping refinement for '{tool_name}' - Reason: {stop_reason}."
                    )
                    await _emit_progress(
                        {
                            "tool_name": tool_name,
                ...
                await _emit_progress(
                    {
                        ...
                        "iteration": iteration,
                        "stage": "iteration_complete",
                        "message": "Preparing for next iteration",
                    }
                )
        ...
        if tool_iterations_results:
            ...
            final_schema = (
                current_schema_for_iter  # This holds the state after the last iter's patch
            )
            improvement = (
                ((final_success_rate - initial_success_rate) / (1.0001 - initial_success_rate))
                if initial_success_rate >= 0
                and final_success_rate >= 0
                and initial_success_rate < 1
                else 0.0
            )
            ...
            final_tool_result = RefinedToolResult(
                tool_name=tool_name,
                original_schema=original_schema,
                iterations=tool_iterations_results,
                ...
                token_count_change=(
                    count_tokens(original_desc) - count_tokens(final_docs.description)
                ),
            ...
            final_tool_result = RefinedToolResult(
                ...
                original_schema=original_schema,
                final_proposed_changes=ProposedChanges(description=original_desc),
                final_proposed_schema_patches=[],
                final_schema_after_patches=original_schema,
        ...
        await _emit_progress(
            {
                ...
                "stage": "tool_complete",
                "message": "Refinement process finished.",
            }
        )
    ...
    async def safe_process_single_tool(name: str) -> Optional[RefinedToolResult]:
        try:
            ...
            logger.error(
                f"Critical error during processing task for tool '{name}': {e}", exc_info=True
            )
    ...
    results_from_gather = await asyncio.gather(
        *processing_tasks
    )  # Exceptions inside task are handled
    ...
    await _emit_progress(
        {
            ...
            "total_iterations": max_iterations,
            "stage": "tool_complete",
            "message": "Overall refinement process finished.",
