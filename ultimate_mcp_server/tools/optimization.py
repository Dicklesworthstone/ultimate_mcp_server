...
DEFAULT_SPEED_SCORE = (
    3  # Fallback score if measurement is missing/invalid or hardcoded value is missing
)
...
async def estimate_cost(
    ...
    try:
        ...
        logger.warning(
            f"Could not count tokens for model '{model}' (using '{model_name_only}' for tiktoken): {e}. Using rough estimate."
        )
    ...
    if include_output:
        if max_tokens is not None:
            ...
            logger.debug(
                f"max_tokens not provided, estimating output tokens as {estimated_output_tokens}"
            )
    else:
        estimated_output_tokens = 0
    ...
    logger.info(
        f"Estimated cost for model '{model}' (using key '{resolved_model_key}'): ${total_cost:.6f} (In: {input_tokens} tokens, Out: {estimated_output_tokens} tokens)"
    )
    return {
        "cost": total_cost,
        ...
        "rate": {"input": cost_data["input"], "output": cost_data["output"]},
...
async def compare_models(
    ...
    async def get_estimate(model_input_name):  # Use a distinct variable name
        ...
        try:
            ...
            logger.error(
                f"Unexpected error estimating cost for model {model_input_name}: {e}", exc_info=True
            )
...
async def recommend_model(
    ...
    if priority not in ["cost", "quality", "speed", "balanced"]:
        raise ToolInputError(
            f"Invalid priority: '{priority}'. Must be cost, quality, speed, or balanced."
        )
    ...
    if os.path.exists(filepath):
        try:
            ...
            logger.warning(
                f"Could not load or parse measured speed data from {filepath}: {e}. Speed data will be 0.",
                exc_info=True,
            )
            measured_speeds = {}
    else:
    ...
    model_capabilities = {
        ...
        "openai/gpt-4o": [
            ...
            "math",
            "multimodal",
        ],  # Assuming multimodal based on general knowledge
        ...
        "gemini/gemini-2.5-pro-preview-03-25": [
            ...
            "math",
            "multimodal",
        ],  # Map from gemini-2.5-pro-preview-03-25
        ...
        "openrouter/mistralai/mistral-nemo": [
            ...
            "instruction-following",
            "coding",
        ],  # Estimate based on Mistral family
    }
    ...
    all_prefixed_metadata_keys = (
        set(model_capabilities.keys())
        | set(model_speed_fallback.keys())
        | set(model_quality.keys())
    )
        ...
        if "/" in key:
            ...
            if short_name in short_to_prefixed_map:
                ...
                if short_name not in ambiguous_short_names:
                    logger.warning(
                        f"Ambiguous short model name '{short_name}' found. Maps to '{short_to_prefixed_map[short_name]}' and '{key}'. Will require full name for this model."
                    )
    ...
    def _get_prefixed_name_for_cost_key(cost_key: str) -> Optional[str]:
        ...
        if "/" in cost_key:
            if cost_key in all_prefixed_metadata_keys:
                ...
                logger.warning(
                    f"Prefixed cost key '{cost_key}' not found in any known metadata (capabilities, quality, speed)."
                )
        ...
        if prefixed_name is not None:  # Found unique mapping
            ...
            logger.warning(
                f"Cannot resolve ambiguous short name '{cost_key}'. Please use the full 'provider/model_name' identifier."
            )
            _prefixed_name_cache[cost_key] = None
            return None
        else:  # Short name not found in any metadata
            logger.warning(
                f"Short name cost key '{cost_key}' not found in any known model metadata. Cannot determine provider/full name."
            )
            _prefixed_name_cache[cost_key] = None
            return None
    ...
    async def evaluate_model(cost_key: str):
        ...
        if not prefixed_model_name:
            excluded_models_reasons[cost_key] = (
                "Could not reliably determine provider/full name for metadata lookup."
            )
            return
        ...
        if missing_caps:
            excluded_models_reasons[prefixed_model_name] = (
                f"Missing required capabilities: {missing_caps}"
            )
            return
        ...
        try:
            ...
            logger.error(
                f"Unexpected error estimating cost for {cost_key} (prefixed: {prefixed_model_name}) in recommendation: {e}",
                exc_info=True,
            )
            excluded_models_reasons[prefixed_model_name] = (
                f"Cost estimation failed unexpectedly: {str(e)}"
            )
            return
        ...
        if max_cost is not None and estimated_cost_value > max_cost:
            excluded_models_reasons[prefixed_model_name] = (
                f"Exceeds max cost (${estimated_cost_value:.6f} > ${max_cost:.6f})"
            )
            return
        ...
        logger.debug(
            f"Speed for {prefixed_model_name}: {measured_tps:.1f} t/s (Source: {speed_source})"
        )
    ...
    if not candidate_models_data:
        ...
        for model_data in candidate_models_data:
            ...
            model_data["score"] = calculate_score(
                model_data, min_cost, cost_range, min_tps, tps_range
            )
    ...
    logger.info(
        f"Recommended models (priority: {priority}): {[r['model'] for r in recommendations_list]}"
    )
    return {
...
async def execute_optimized_workflow(
    ...
    try:
        ...
        if api_meta_tool:  # Only add if instance is available
            ...
            logger.warning(
                "APIMetaTool instance not available in execute_optimized_workflow. Meta API tools will not be callable in workflows."
            )
            meta_api_tools = {}
    except ImportError:
        logger.warning(
            "APIMetaTool not found (meta_api_tool.py). Meta API tools cannot be used in workflows."
        )
        meta_api_tools = {}
    ...
    try:
        ...
        logger.warning(
            "entity_relation_graph module not found. extract_entity_graph will not be available in workflows."
        )
    ...
    for i, stage in enumerate(workflow):
        ...
        if not all(k in stage for k in ["stage_id", "tool_name", "params"]):
            raise ToolInputError(
                f"Workflow stage {i} missing required keys (stage_id, tool_name, params)."
            )
    ...
    async def execute_stage(stage_id: str) -> None:
        ...
        async with concurrency_semaphore:
            ...
            try:
                        ...
                        async def run_iteration(item_idx, item_value):
                            async with concurrency_semaphore:
                                ...
                                try:
                                    ...
                                    error_msg = (
                                        f"Iteration {item_idx} failed: {type(e).__name__}: {str(e)}"
                                    )
                ...
                error_msg = (
                    f"Workflow failed at stage '{stage_id}'. Error: {type(e).__name__}: {str(e)}"
                )
                logger.error(error_msg, exc_info=True)
                stage_results[stage_id] = {"error": error_msg, "traceback": traceback.format_exc()}
    ...
    async def execute_dag() -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Workflow execution order (respecting dependencies): {execution_order}"
                )
            except nx.NetworkXUnfeasible as e:
                # Should never happen as we already checked for cycles
                raise ToolInputError(
                    "Workflow contains circular dependencies that were not detected earlier."
                ) from e
            ...
            while len(completed_stages) < len(dag):
                ...
                ready_stages = [
                    ...
                    if (
                        stage_id not in completed_stages
                        and all(pred in completed_stages for pred in dag.predecessors(stage_id))
                    )
                ]
                ...
                if not ready_stages:
                    if len(completed_stages) < len(dag):
                        ...
                        raise ToolError(
                            "Workflow execution stalled due to unresolvable dependencies."
                        )
                    break
                ...
                try:
                    ...
                    failed_stage = next(
                        (
                            ...
                            if s in stage_results and "error" in stage_results[s]
                        ),
                        ready_stages[0],  # Fallback if we can't identify the specific failed stage
                    )

                    error_info = stage_results.get(failed_stage, {}).get(
                        "error", f"Unknown error in stage '{failed_stage}'"
                    )
    ...
    if result["success"]:
        ...
        logger.error(
            f"Workflow failed after {total_time:.2f}s: {result.get('error', 'Unknown error')}"
        )
...
def _resolve_params(
    ...
    if iterate_on_ref:
        ...
        if ref_key == "documents":
            if documents is None:
                raise ValueError(
                    f"Stage '{stage_id}' iterates on documents, but no documents were provided."
                )
            ...
            if not isinstance(dep_output, dict) or output_key not in dep_output:
                raise ValueError(
                    f"Output key '{output_key}' not found in dependency '{dep_stage_id}' for iteration."
                )
        ...
        logger.debug(
            f"Stage '{stage_id}' will iterate over {len(iteration_list)} items from '{iterate_on_ref}'"
        )
    ...
    for key, value in params.items():
        if isinstance(value, str) and value.startswith("${") and value.endswith("}"):
            ...
            if ref_key == "documents":
                if documents is None:
                    raise ValueError(
                        f"Parameter '{key}' references documents, but no documents provided."
                    )
                ...
                if dep_stage_id not in stage_results or "output" not in stage_results[dep_stage_id]:
                    raise ValueError(
                        f"Dependency '{dep_stage_id}' for parameter '{key}' not found or failed."
                    )
                dep_output = stage_results[dep_stage_id]["output"]
                # Handle potential nested keys in output_key later if needed
                if not isinstance(dep_output, dict) or output_key not in dep_output:
                    raise ValueError(
                        f"Output key '{output_key}' not found in dependency '{dep_stage_id}' for parameter '{key}'. Available keys: {list(dep_output.keys()) if isinstance(dep_output, dict) else 'N/A'}"
                    )
    ...
    if is_iteration and iteration_param_name is None:
        ...
        raise ValueError(
            f"Iteration target '{iterate_on_ref}' must correspond to a parameter value in stage '{stage_id}'."
        )
...
def _inject_iteration_item(base_params: Dict, item: Any) -> Dict:
    ...
    if iter_param_name:
        ...
        logger.error(
            "Cannot inject iteration item: Iteration parameter name not found in resolved params."
        )
...
async def _gather_iteration_results(stage_id: str, tasks: List[asyncio.Task]) -> List[Any]:
    ...
    try:
        ...
        logger.error(
            f"Iteration stage '{stage_id}' failed: One or more sub-tasks raised an error.",
            exc_info=True,
        )
