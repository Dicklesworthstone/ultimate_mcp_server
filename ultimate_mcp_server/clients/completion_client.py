...
class CompletionClient:
    ...
    def __init__(
        self, default_provider: str = Provider.OPENAI.value, use_cache_by_default: bool = True
    ):
    ...
    async def generate_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            if use_cache and self.cache_service.enabled:
                await self.cache_service.set(key=cache_key, value=result, ttl=cache_ttl)
    ...
    async def try_providers(
        self, prompt: str, providers: List[str], models: Optional[List[str]] = None, **kwargs
    ):
        """
        ...
        for i, provider_name in enumerate(providers):
            try:
                ...
                result = await self.generate_completion(
                    prompt=prompt, provider=provider_name, model=models[i], **kwargs
                )
                return result
            except Exception as e:
