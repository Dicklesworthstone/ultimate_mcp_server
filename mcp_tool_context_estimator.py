    ...
    try:
        if os.path.exists(filename):
            with open(filename, "r") as f:
                ...
                if not quiet:
                    console.print(
                        f"[green]Successfully loaded {len(tool_data)} tools from {filename}[/green]"
                    )
                return tool_data
        else:
            if not quiet:
                console.print(
                    f"[yellow]Tool list file {filename} not found. Will use server-provided tools only.[/yellow]"
                )
            return []
...
async def detect_server_transport(host: str, port: str, quiet: bool = False) -> tuple[str, str]:
                        ...
                        if response.status == 200:
                            ...
                            try:
                                ...
                                if '"jsonrpc":"2.0"' in data or '"result"' in data:
                                    if not quiet:
                                        console.print(
                                            f"[green]Detected {transport} transport at {url}[/green]"
                                        )
                            ...
                            if not quiet:
                                console.print(
                                    f"[dim]Endpoint {url} returned {response.status}[/dim]"
                                )
                            continue
                else:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(f"http://{host}:{port}/") as response:
                if response.status == 200:
                    ...
                    if not quiet:
                        console.print(
                            f"[yellow]Server detected but transport unclear, defaulting to streamable-http at {default_url}[/yellow]"
                        )
    ...
    if not quiet:
        console.print(
            f"[yellow]Could not detect transport mode, defaulting to SSE at {fallback_url}[/yellow]"
        )
...
async def get_mcp_server_tools_streamable_http(
    server_url: str,
    ...
    if not quiet:
        console.print(
            f"[bold blue]Connecting to streamable-http MCP server at {server_url}...[/bold blue]"
        )
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            if not quiet:
                console.print(
                    "[bold blue]Initializing MCP protocol via streamable-http...[/bold blue]"
                )
            ...
            async with session.post(server_url, json=init_data, headers=headers) as response:
                ...
                if not quiet:
                    console.print(
                        f"[green]Connected to server:[/green] {server_name} v{server_version}"
                    )
                ...
                if not quiet and not has_tools:
                    console.print(
                        "[bold yellow]Warning: This server does not advertise tools capability![/bold yellow]"
                    )
                    console.print(
                        "The server might not support tool listing, but we'll try anyway."
                    )
                ...
                if server_instructions and not quiet:
                    console.print(
                        f"[green]Server provides instructions of length {len(server_instructions):,} chars[/green]"
                    )
                elif not quiet:
            ...
            init_notify_data = {"jsonrpc": "2.0", "method": "notifications/initialized"}
            ...
            list_tools_data = {"jsonrpc": "2.0", "id": 2, "method": "tools/list"}
            ...
            async with session.post(server_url, json=list_tools_data, headers=headers) as response:
                ...
                for tool in tools:
                    ...
                    tool_dict = {"name": tool.get("name"), "inputSchema": tool.get("inputSchema")}
                    ...
                    if tool.get("description"):
                        ...
                        if not quiet:
                            console.print(
                                f"[dim]Tool '{tool['name']}' has description ({len(desc):,} chars)[/dim]"
                            )
                        tool_dict["description"] = desc
                    elif not quiet:
                        console.print(
                            f"[dim yellow]Tool '{tool['name']}' has no description[/dim yellow]"
                        )
                ...
                if not quiet:
                    console.print(
                        f"[green]{has_descriptions} out of {tool_count} tools have descriptions[/green]"
                    )
                    if has_descriptions > 0:
                        console.print(
                            f"[green]Average description length: {total_desc_length / has_descriptions:,.1f} chars[/green]"
                        )
        ...
        console.print(
            f"[bold red]Error connecting to streamable-http MCP server:[/bold red] {str(e)}"
        )
        if not quiet:
...
async def get_mcp_server_tools_stdio(
    command: str,
    ...
    if not quiet:
        console.print(
            f"[bold blue]Connecting to MCP server via stdio: {command} {' '.join(args or [])}[/bold blue]"
        )
            ...
            async with ClientSession(read, write) as session:
                ...
                try:
                    ...
                    if hasattr(tools_result, "tools"):
                        ...
                        if not quiet:
                            console.print(
                                "[bold yellow]Tools result doesn't have expected structure. Trying alternatives...[/bold yellow]"
                            )
                        if hasattr(tools_result, "__iter__"):
                            ...
                            if not quiet:
                                console.print(
                                    f"[bold yellow]Tools result type: {type(tools_result)}[/bold yellow]"
                                )
...
async def get_mcp_server_tools(
    ...
    try:
            ...
            async with ClientSession(read, write) as session:
                ...
                try:
                    ...
                    if not quiet:
                        try:
                            console.print(
                                "[bold blue]Getting additional server information...[/bold blue]"
                            )
                            if (
        ...
        if "Connection refused" in str(e):
            console.print(
                "[bold yellow]The server doesn't appear to be running at the specified URL.[/bold yellow]"
            )
            console.print(
                "Make sure your MCP server is running and available at the URL you specified."
            )
        elif "401" in str(e):
            console.print(
                "[bold yellow]Authentication error - the server requires credentials.[/bold yellow]"
            )
        elif "404" in str(e):
...
def create_full_tool_registration_prompt(server_info, tools=None, quiet=False):
    ...
    for tool in tools:
        ...
        tool_dict_for_api = {"name": tool["name"], "input_schema": tool["inputSchema"]}
    ...
    console.print(
        f"[green]Included {desc_count} descriptions out of {len(tools)} tools in final output[/green]"
    )
    if desc_count > 0:
        console.print(
            f"[green]Average description length in final output: {total_desc_len / desc_count:,.1f} chars[/green]"
        )
...
def analyze_tools_token_usage(
    current_tools: Dict[str, Any], all_tools: Dict[str, Any], quiet: bool = False
):
    """
    ...
    if same_toolsets and not quiet:
        ...
        console.print(
            "[yellow]This suggests the server is already running with --load-all-tools[/yellow]"
        )
    ...
    current_tools_prompt = create_full_tool_registration_prompt(
        current_tools, current_tools_subset, quiet
    )
    ...
    current_tools_costs = {
        model: (price * current_tools_prompt_tokens / 1000) for model, price in MODEL_PRICES.items()
    }
    all_tools_costs = {
        model: (price * all_tools_prompt_tokens / 1000) for model, price in MODEL_PRICES.items()
    }
    ...
    count_percentage = (
        (data["current_tools"]["count"] / data["all_tools"]["count"]) * 100
        if data["all_tools"]["count"] > 0
        else 100
    )
    ...
    full_size_percentage = (
        (data["current_tools"]["full_size_kb"] / data["all_tools"]["full_size_kb"]) * 100
        if data["all_tools"]["full_size_kb"] > 0
        else 100
    )
    ...
    full_tokens_percentage = (
        (data["current_tools"]["full_tokens"] / data["all_tools"]["full_tokens"]) * 100
        if data["all_tools"]["full_tokens"] > 0
        else 100
    )
...
async def get_complete_toolset(quiet: bool = False) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        current_tools_info = await get_mcp_server_tools(
            server_url, transport_type, quiet=quiet, command=None, args=None
        )
        current_tools = (
            {tool["name"]: tool for tool in current_tools_info["tools"]}
            if current_tools_info
            else {}
        )
        if not quiet:
            console.print(
                f"[green]Retrieved {len(current_tools)} tools from current server to use their real descriptions[/green]"
            )
    except Exception as e:
        if not quiet:
    ...
    if not all_tool_names and current_tools:
        if not quiet:
            console.print(
                "[yellow]No tools found in file. Using current server tools and adding some common ones.[/yellow]"
            )
        ...
        additional_tools = [
            ...
            "smart_browser.autopilot",
            "smart_browser.parallel",
            "smart_browser.download_site_pdfs",
            "generate_image",
            "analyze_image",
            "transcribe_audio",
        ]
    ...
    if not quiet:
        console.print(
            f"[green]Using complete list of {len(all_tool_names)} tools for all-tools mode[/green]"
        )
    ...
    for tool_name in all_tool_names:
        ...
        if tool_name in current_tools:
            ...
            tool_desc = (
                f"The {tool_name} tool provides functionality for {tool_name.replace('_', ' ')}. "
            ...
            if not quiet:
                console.print(
                    f"[dim yellow]Created placeholder for tool '{tool_name}'[/dim yellow]"
                )
...
def parse_args():
    ...
    parser.add_argument(
        "--url", default=None, help="URL of the MCP server (default: auto-detected)"
    )
    parser.add_argument(
        "--transport",
        default=None,
    ...
    parser.add_argument(
        "--args", default=None, nargs="*", help="Additional arguments for stdio command"
    )
    parser.add_argument(
        "--no-all-tools", action="store_true", help="Skip comparison with all tools"
    )
    parser.add_argument(
        "--quiet",
        "-q",
...
async def main():
    ...
    if args.transport == "stdio":
        ...
        if not args.quiet:
            console.print(
                f"[blue]Using stdio transport with command: {command} {' '.join(stdio_args)}[/blue]"
            )
    else:
        ...
        if args.url:
            ...
            if args.transport:
                ...
                server_url, transport_type = await detect_server_transport(
                    host, port, quiet=args.quiet
                )
        else:
    ...
    try:
        ...
        current_tools = await get_mcp_server_tools(
            server_url, transport_type, quiet=quiet_mode, command=command, args=stdio_args
        )
        ...
        if args.no_all_tools:
            ...
            if not quiet_mode:
                ...
                console.print(
                    "[green]Creating an artificial subset of current tools for comparison[/green]"
                )
            ...
            if not quiet_mode:
                console.print(
                    f"[green]Created subset with {subset_size} tools out of {total_tools} total[/green]"
                )
            ...
            if abs(current_tool_count - all_tool_count) <= 2:  # Allow small difference
                if not quiet_mode:
                    console.print(
                        f"[yellow]Warning: Current server has {current_tool_count} tools, "
